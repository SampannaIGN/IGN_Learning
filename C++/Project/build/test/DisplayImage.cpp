...
char img_path[] = "/home/sampannat/Downloads/Learning/C++/Project/build/test/Image256X256.jpg";//"/home/edgeai/code/b7932272-8e55-74c7-3eb5-acfee9cfed10.png";
 ...
 */
{
    ...
    ulUpper =  ulClipLimit - ulBinIncr;	/*values must be atleat ulUpper size in each bin*/ 
...
/*
if we have elements from 10-50(min,max) and we have to categorize them in 5(uiNrBins) bins
range of each bin is Bin Size
i.e BinSize = 1+(50-10)/5 =9
which means mapping is done as follows
10-18 => 0
19-27 => 1
28-36 => 2
37-45 => 3
46-50 => 4
In simple words it tells you to which bin(0-(uiNrBins-1)) does a pixel value exit
*/
void MakeLut (kz_pixel_t * pLUT, kz_pixel_t Min, kz_pixel_t Max, unsigned int uiNrBins)
 ...
 */
{
    ...
    std::cout<<"Bin size = "<<(unsigned int)BinSize<<endl;
    for (i = Min; i <= Max; i++) 
    {   
...
{   
    ...
    //convert BGR image to gray scale image & stores in bgr_image
    ...
    //        std::cout<<(unsigned int)img_array[i][j]<<" ("<<i<<","<<j<<") "<<endl;
    //    }
    // }
...
//int result = CLAHE_CUSTOM(gray_image.data,img_width,img_height,0,255,16,5,256,40);
    int result = CLAHE_CUSTOM(gray_image.data,img_width,img_height,0,255,16,16,256,100);
